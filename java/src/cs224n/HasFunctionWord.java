 ...
 * A rule featurizer.
 */
public class HasFunctionWord implements RuleFeaturizer<IString, String> {
  ...
  public List<FeatureValue<String>> ruleFeaturize(
    ...
    if (f.targetPhrase != null && f.targetPhrase.size() > 0) {
      for (IString currIWord : f.targetPhrase) {
        String currWord = currIWord.toString();
        if (functionWords.contains(currWord)) {
